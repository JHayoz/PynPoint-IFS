...
class IfuFakePlanetModule(ProcessingModule):
    """
    ...
    def __init__(self,
        ...
        super(IfuFakePlanetModule, self).__init__(name_in)

        self.m_image_in_port = self.add_input_port(image_in_tag)
        self.m_contrast_in_port = self.add_input_port(magnitude_in_tag)
        ...
        self.m_parang_cube = parang_cube
    ...
    def run(self) -> None:
        ...
        sep = np.ones(len(nspectrum))*self.m_position[0]
        ang = np.radians(np.ones(len(nspectrum))*self.m_position[1] + 90. - self.m_parang_cube)
        
        x_shift_pl = sep*np.cos(ang)
        y_shift_pl = sep*np.sin(ang)
        shift_pl = np.array([y_shift_pl,x_shift_pl]).T
        ...
        if type(self.m_shift_cubes_in_tag) == str:
            ...
            shift_x = np.zeros(len(self.m_parang_cube))
            shift_y = np.zeros(len(self.m_parang_cube))
            count = 0
        ...
        shift_tot = shift+shift_pl
        
        sep_tot = (np.sqrt(shift_tot[:,0]**2+shift_tot[:,1]**2))[0]
        ang_tot = math.atan2(shift_tot[:,0], shift_tot[:,1])
        ang_tot = (math.degrees(ang_tot) - 90) % 360

        images = self.m_image_in_port.get_all()
        psf = self.m_psf_in_port.get_all()

        self.m_magnitude = self.m_contrast_in_port[:,0]
        self.m_psf_scaling = self.m_contrast_in_port[:,1]
        
        start_time = time.time()
        for j, _ in enumerate(range(im_shape[0])):
            progress(j, im_shape[0], 'Injecting artificial planets...', start_time)

            im_fake = fake_planet(images=np.expand_dims(images[j], axis=0),
                                  psf=np.expand_dims(psf[j], axis=0),
                                  parang=np.array([0]),
                                  position=(sep_tot, ang_tot),
                                  magnitude=self.m_magnitude[j],
                                  psf_scaling=self.m_psf_scaling[j],
                                  interpolation='spline')

            self.m_image_out_port.append(im_fake, data_dim=3)

        history = f'(sep, angle, mag) = ({self.m_position[0]*pixscale:.2f}, ' \
                  f'{self.m_position[1]:.2f}, {np.mean(self.m_magnitude):.2f})'
...
class CreatePlanetBTSettlContrastModule(ProcessingModule):
    """
        Pipeline module to inject a positive or negative artificial planet into a stack of images.
        """
    ...
    def __init__(self,
                 name_in: str,
                 stellar_spectrum_in_tag: Tuple[str, float],
                 wv_in_tag: str,
                 contrast_out_tag: str,
                 file_in: str,
                 star_prop: Tuple[float, float, float] = (10.,4000,1.),
                 R: float = 1.,
                 RV:float = 0.,
                 interpolation: str = 'spline') -> None:
        """
        ...
        self.m_stellar_spectrum_in_port = self.add_input_port(stellar_spectrum_in_tag[0])
        ...
        self.m_radius_ap = stellar_spectrum_in_tag[1]
        self.m_d = star_prop[0]
        self.m_T_star = star_prop[1]
        self.m_R_star = star_prop[2]
        self.m_R = R
        self.m_RV = RV
    ...
    def run(self) -> None:
        ...
        def _planck(wavel_points,
           ...
           scaling):
            ...
            planck_1 = 2.*PLANCK*LIGHT**2/(1e-6*wavel_points)**5
    
            planck_2 = np.exp(PLANCK*LIGHT /
                      (1e-6*wavel_points*BOLTZMANN*temperature)) - 1.
        ...
        self.m_R = self.m_R * R_JUP
        wv = self.m_wv_in_port.get_all()
        
        sys.stdout.write('Creating the BT-Settl resampled spectrum...')
        sys.stdout.flush()
        
        dataf = pd.pandas.read_csv(self.m_file_in,
                           usecols=[0, 1],
        ...
        data_flux = data_flux*1e4*1e-7*1e4*(self.m_R**2)/(self.m_d*PARSEC)**2
        data_flux_RV, _ = dopplerShift(data_wavel, data_flux, self.m_RV, edgeHandling='firstlast')
        
        # [Angstrom] -> [micron]
        data_wavel = data_wavel*1e-4
        
        bt_settl_datares = spectres(wv, data_wavel, data_flux_RV)
    
        sys.stdout.write('\rCreating the BT-Settl resampled spectrum --> [DONE]\n')
        sys.stdout.flush()
        
        
        sys.stdout.write('Calculating the stellar spectrum...')
        images = self.m_stellar_spectrum_in_port.get_all()
        position = center_subpixel(images[0, ])
        stellar_spectrum_data = []
        aperture = CircularAperture((position[1], position[0]), self.m_radius_ap)
        
        for i in range(len(images)):
            stellar_spectrum_data.append(aperture_photometry(images[i], aperture, method='exact')['aperture_sum'])
        stellar_spectrum_data = np.array(stellar_spectrum_data).T
        
        stellar_spectrum_planck = _planck(wv, self.m_T_star,((self.m_R_star*R_SUN)**2)/(self.m_d*PARSEC)**2)

        sys.stdout.write('\rCalculating the stellar spectrum --> [DONE]\n')
        sys.stdout.flush()

        planet_spectrum_data = stellar_spectrum_data / stellar_spectrum_planck * bt_settl_datares

        contrast_spectrum = planet_spectrum_data / stellar_spectrum_data
        print (-2.5*np.log10(np.sum(planet_spectrum_data)/np.sum(stellar_spectrum_data)))

        contrast_output = np.hstack(((-2.5*np.log10(np.abs(contrast_spectrum))).T, np.sign(contrast_spectrum).T))
        self.m_contrast_out_port.set_all(contrast_output)
        
        history = 'BT-Settl = ' 'ciao'
        
        self.m_contrast_out_port.copy_attributes(self.m_stellar_spectrum_in_port)
        self.m_contrast_out_port.add_history('CreatePlanetBTSettlContrastModule', history)
...
class CreatePlanetpTContrastModule(ProcessingModule):
    ...
    def __init__(self,
                 ...
                 contrast_out_tag: str,
                 pT: str,
                 star_prop: Union[Tuple[float, float, float], str] = 10.,
                 Dist: float = 10.,
                 R: float = 1.,
                 RV:float = 0.,
                 species: dict ={},
        ...
        self.m_radius_ap = stellar_spectrum_in_tag[1]
        self.m_d = Dist
        if type(star_prop)==str:
            self.m_adress = star_prop
            self.m_BB = False
        else:
            self.m_T_star = star_prop[0]
            self.m_R_star = star_prop[0]
            self.m_BB = True
        self.m_R = R
        self.m_RV = RV
        self.m_species = species
    ...
    def run(self) -> None:
        ...
        def _planck(wavel_points,
            ...
            planck_2 = np.exp(PLANCK*LIGHT/(1e-6*wavel_points*BOLTZMANN*temperature)) - 1.
        ...
        sys.stdout.write('Creating the petitRADTRANS resampled spectrum...')
        sys.stdout.flush()
        
        
        table = np.loadtxt(self.m_pT)
        p = table[:,0]
        T = table[:,1]
        print (self.m_pT[-42])

        if self.m_pT[-42]=='/':
            g = float(self.m_pT[-23:-19])
            M = float(self.m_pT[-14:-10])
            Teff = float(self.m_pT[-32:-29])
        else:
            g = float(self.m_pT[-23:-19])
            M = float(self.m_pT[-14:-10])
            Teff = float(self.m_pT[-33:-29])
    
        nspec = len(self.m_species.keys())
        spec_names = self.m_species.keys()
        ...
        rad = rt.Radtrans(line_species= other_moc,
                  rayleigh_species= ray_spec,
                  continuum_opacities = con_op,
                  wlen_bords_micron=[wv[0]-0.1, wv[-1]+0.1],
                  mode = 'lbl')

        rad.setup_opa_structure(p)


        mmw = []
        abunds = {}
        for k in spec_names:
            mmw.append(self.m_species[k][0]*self.m_species[k][1])
        MMW = np.ones_like(p)*np.sum(mmw)

        for i, k in enumerate(spec_names):
            abunds[k] = np.ones_like(p)*mmw[i]/MMW

        rad.calc_flux(T, abunds, 10**g, MMW)

        # F_nu --> f_lambda in W/m^2/micron
        wv_HR = nc.c/rad.freq/1e-4
        data_flux_HR = rad.flux*3e8/(wv_HR**2*1e-6)*1e-7*1e4*(self.m_R**2)/(self.m_d*PARSEC)**2

        data_flux_RV, _ = dopplerShift(wv_HR, data_flux_HR, self.m_RV, edgeHandling='firstlast')
        pT_datares = spectres(wv, wv_HR, data_flux_RV)

                                   
        sys.stdout.write('\rCreating the petitRADTRANS resampled spectrum --> [DONE]\n')
        sys.stdout.flush()
        ...
        if self.m_BB:
            stellar_spectrum_planck = _planck(wv, self.m_T_star,((self.m_R_star*R_SUN)**2)/(self.m_d*PARSEC)**2)
        elif self.m_BB==False:
            #file = np.loadtxt(self.m_adress)
            #stellar_spectrum_planck = spectres(wv, file[:,0], file[:,1])
            stellar_spectrum_planck = np.loadtxt(self.m_adress)[:,1]
        print (stellar_spectrum_planck)

        sys.stdout.write('\rCalculating the stellar spectrum --> [DONE]\n')
        sys.stdout.flush()


        planet_spectrum_data = stellar_spectrum_data / stellar_spectrum_planck * pT_datares
        ...
        history = 'PT radtrans = ' 'ciao'
        
        self.m_contrast_out_port.copy_attributes(self.m_stellar_spectrum_in_port)
        self.m_contrast_out_port.add_history('CreatePlanetpTContrastModule', history)
        self.m_contrast_out_port.close_port()
...
class MCMCsamplingFixPositionModule(ProcessingModule):
    """
    ...
    def __init__(self,
        ...
        super(MCMCsamplingFixPositionModule, self).__init__(name_in)
    ...
    def run(self) -> None:
        ...
        if isinstance(self.m_aperture, float):
            yx_pos = polar_to_cartesian(images, self.m_pos[0]/pixscale, self.m_pos[1])
        ...
        initial[:, 0] = self.m_param + np.random.normal(0, self.m_sigma, self.m_nwalkers)
        ...
        mag_percen = np.percentile(samples[:, 0], [16., 50., 84.])
...
class SimplexMinimizationFixPositionModule(ProcessingModule):
    """
    ...
    def __init__(self,
        ...
        super(SimplexMinimizationFixPositionModule, self).__init__(name_in)
    ...
    def run(self) -> None:
        ...
        aperture = (int(round(self.m_position[1])), int(round(self.m_position[0])), self.m_aperture/pixscale)
        ...
        def _objective(arg, pos_y, pos_x, count, n_components, sklearn_pca, noise):
            mag = arg[0]
        ...
        for i, n_components in enumerate(self.m_pca_number):
            ...
            minimize(fun=_objective,
                    x0=[self.m_magnitude],
                    args=(pos_init[0], pos_init[1],i, n_components, sklearn_pca, noise),
                    method='Nelder-Mead',
                    tol=None,
...
class SystematicErrorFixPositionModule(ProcessingModule):
    """
    ...
    def __init__(self,
        ...
        super(SystematicErrorFixPositionModule, self).__init__(name_in)
    ...
    def run(self) -> None:
        ...
        for i, ang in enumerate(angles):
            ...
            module = SimplexMinimizationFixPositionModule(position=position,
                                                            magnitude=self.m_magnitude,
                                                            psf_scaling=-self.m_psf_scaling,
                                                            name_in=f'{self._m_name}_fake_{i}',
